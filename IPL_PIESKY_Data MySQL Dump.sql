...
select ibd.bidder_id,ibd.bidder_name, ibp.no_of_bids, count(ibd2.bid_status), ((count(ibd2.bid_status)/ibp.no_of_bids )*100) from ipl_bidder_details ibd join ipl_bidder_points ibp on ibd.bidder_id=ibp.bidder_id
join ipl_bidding_details ibd2 on ibp.bidder_id=ibd2.bidder_id
where bid_status like 'Won'
group by bidder_id order by ((count(ibd2.bid_status)/ibp.no_of_bids )*100) desc;
##2.Display the number of matches conducted at each stadium with stadium name, city from the database.
select ipls.stadium_id,ipls.stadium_name,ipls.city, count(ipls.stadium_id) from ipl_stadium ipls join ipl_match_schedule iplm on ipls.stadium_id=iplm.stadium_id
group by ipls.stadium_id
order by ipls.stadium_id;
...
select sum(no_of_bids),iplt.team_id,iplt.team_name from ipl_bidder_points iplbp join ipl_bidding_details iplbd on iplbp.bidder_id=iplbd.bidder_id
join ipl_team iplt on iplbd.bid_team=iplt.team_id
group by iplt.team_id,iplt.team_name
order by iplt.team_id;
##5.Show the team id who won the match as per the win details.
Select Team_ID, Team_name, Team_ID1, Team_ID2, Match_Winner,iplm.Win_Details
from IPL_TEAM iplt
join IPL_MATCH iplm
on SUBSTR(iplt.Remarks,1,3) = SUBSTR(iplm.Win_Details,6,3);
##6.Display total matches played, total matches won and total matches lost by team along with its team name.
select iplt.team_id,iplt.team_name,sum(iplts.matches_played),sum(iplts.matches_won) ,sum(iplts.matches_lost)
from ipl_team iplt join ipl_team_standings iplts on iplt.team_id=iplts.team_id
group by iplt.team_id,iplt.team_name;
##7.Display the bowlers for Mumbai Indians team
select iplp.player_id,iplp.player_name,ipltp.player_role,iplt.team_name from ipl_team iplt join ipl_team_players ipltp on iplt.team_id=ipltp.team_id
join ipl_player iplp on ipltp.player_id=iplp.player_id
where ipltp.player_role ='Bowler' and iplt.team_name like '%mumbai%'
group by iplp.player_name,iplp.player_id;
##8.How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.
select iplt.team_name, ipltp.player_role, count(player_role) 
from ipl_team iplt join ipl_team_players ipltp
on iplt.team_id=ipltp.team_id
group by ipltp.player_role, iplt.team_name
having ipltp.player_role = 'all-rounder' and count(ipltp.player_role) > 4
order by count(ipltp.player_role)>4 desc;
